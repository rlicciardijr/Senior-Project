...
############################################################################
fit2 <- lm(V1 ~ V3, data = days)
summary(fit2)

ggplot(days, aes(factor(Date), Freq)) + geom_point() + geom_smooth(method = "lm") + theme_classic()+scale_x_discrete(labels= (1:15))
...
#####################################################
ggplot(data = mf) +
  geom_bar(aes(mf,x = mf$Rev, x = mf$Title, y = mf$Hash, fill = Hash)) + theme_classic()+xlab("Movie title Hashtags") +
